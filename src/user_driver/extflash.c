...
void Spi_WriteByte(uint8_t var)
{
	...
	for (i = 0; i < 8; ++i)
	{
		...
		nrf_gpio_pin_write(FLASH_SCLK_PIN, );
...
{
	nrf_gpio_cfg_output(FLASH_POWER_PIN);
	nrf_gpio_cfg_output(FLASH_CS_PIN);
	nrf_gpio_cfg_output(FLASH_SDI_PIN);
	nrf_gpio_cfg_output(FLASH_SCLK_PIN);
	nrf_gpio_cfg_output(FLASH_SDO_PIN);

	nrf_gpio_pin_write()
	nrf_gpio_pin_write(FLASH_CS_PIN, 0);
	nrf_gpio_pin_write(FLASH_SDI_PIN, 0);
	nrf_gpio_pin_write(FLASH_SCLK_PIN, 0);
	nrf_gpio_pin_write(FLASH_SDO_PIN, 0);
...
void BspPowerOnExtflash(void)
{
	...
	nrf_gpio_pin_write(FLASH_POWER_PIN, 0);
